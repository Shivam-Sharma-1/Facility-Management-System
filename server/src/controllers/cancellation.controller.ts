...
export const requestCancellation: RequestHandler = async (
  ...
  try {
    ...
    if (!user || !booking) {
      return next(
        createHttpError.InternalServerError("Unable to request cancellation")
      );
    }
...
export const getAllCancellationRequestsGD: RequestHandler = async (
  ...
  try {
    ...
    if (!cancellationRequests) {
      return next(
        createHttpError.Forbidden("You do not have access to this resource")
      );
    }
...
export const approveCancellationRequestGD: RequestHandler = async (
  ...
  try {
    ...
    if (!cancellationRequest) {
      return next(createHttpError.NotFound("Cancellation request not found"));
    }
